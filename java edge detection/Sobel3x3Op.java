...
/* Uses the Sobel 3x3 operator for edge detection
 */
 
public class Sobel3x3Op{
	 ...
	 */
	public Sobel3x3Op(Image p_originalImage){
	...
	/* implements sobel 3x3 algorithm
	 */
	private void edgeAlgorithm(int[] processedpixels){
		...
		//do sobel 3x3 operator
		int finalmatrix[][] = new int[height][width];
		for(int r = 1; r < height - 1; r++){
			for(int c = 1; c < width - 1; c++){
				int xtemp = colormatrix[r-1][c-1] + 
							2*colormatrix[r][c-1] + 
							colormatrix[r+1][c-1] - 
							2*colormatrix[r-1][c-1] -
							colormatrix[r][c] - 
							colormatrix[r+1][c+1];
				int ytemp = colormatrix[r-1][c-1] + 
							2*colormatrix[r-1][c] + 
							colormatrix[r-1][c+1] - 
							colormatrix[r+1][c-1] -
							2*colormatrix[r+1][c] - 
							colormatrix[r+1][c+1];
				finalmatrix[r][c] = (int) Math.sqrt(xtemp*xtemp + ytemp*ytemp);

				int k = finalmatrix[r][c]; // (Math.abs(finalmatrix[r][c] %255)) & 0xff;
				//convert back to array
				int newrgb = (0xff000000 | (k << 16) | (k << 8) | k);
				processedpixels[r*width+c] = newrgb;
				//System.out.print((k << 16) + " " + (k << 8) + " " + (k) + "->" + newrgb + " ");
